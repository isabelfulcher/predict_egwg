...
gam <- mgcv::gam(outcome_wg_over ~ 
                   s(slope_visit2) + # tried changing other values; no change
                   age_cat_under20 + 
                   age_cat_30_35 + 
                   age_cat_over35 + 
                   prepreg_bmi_under + 
...
# Coefficients #
data.frame(est = summary(gam)$p.coeff,
           se = summary(gam)$se[1:10]) %>%
  mutate(or_est = round(exp(est),2),
         ci_low = round(exp(est + qnorm(.025)*se),2),
         ci_up = round(exp(est + qnorm(.975)*se),2),
         pval = round(summary(gam)$p.pv,3)) %>%
  select(-est,-se) -> results_coeff

# Create results data frame #  
pred <- mgcv::predict.gam(object=gam,newdata=df_test,type="response")
